...
class VMobject(Mobject):
    ...
    需要研究着色器代码
    猜测: 
    在片段着色器中是可以看见基元的所有点。用像素连接这些点, 就可以得到一条直线。
    当控制像素的数量时, 就可以得到一条宽度不一的直线
    """
    def set_stroke(
        self,
        color: ManimColor | None = None,
    ...
    def add_smooth_cubic_curve_to(self, handle: np.ndarray, point: np.ndarray):
        """
        这里用三阶贝塞尔曲线生成曲线
        三阶曲线需要两个handle(第一个anchor是self.get_points()[-1])
        这里只给出了一个
        需要计算出另一个
        """
        self.throw_error_if_no_points()
    ...
    def insert_n_curves_to_point_list(self, n: int, points: np.ndarray):
        ...
        这个函数的功能不完美
        假设当前的曲线是1/4圆弧, 由两段贝塞尔曲线拼接而成
        每一段是1/8的圆弧
        假设现在要插入一段圆弧
        当前函数的做法: 将1/8圆弧变为两段1/16圆弧
        1/4 = 1/8 + 1/16 + 1/16
        ...
        进一步思考:
        上面举圆弧的例子有点特殊化了
        对于由多段贝塞尔曲线拼接成的任意曲线来说, 只能对每一小段贝塞尔曲线进行插入
        是无法对整个任意曲线进行均匀插入的
        """
        nppc = self.n_points_per_curve
        if len(points) == 1:
